...
const d: BiDict = {
  ...
  deleteConfirm: {
    'zh-CN': '确定删除该白名单车辆吗？删除后不可恢复。',
    en: 'Delete this whitelist vehicle? This cannot be undone.'
  },
  confirm: { 'zh-CN': '确定', en: 'Confirm' },
  cancel: { 'zh-CN': '取消', en: 'Cancel' },
  addTitle: { 'zh-CN': '新增白名单车辆', en: 'New Whitelist Vehicle' },
  editTitle: { 'zh-CN': '编辑白名单车辆', en: 'Edit Whitelist Vehicle' },
  ...
  lblType: { 'zh-CN': '车辆类型', en: 'Vehicle Type' },
  ...
  lblEnabled: { 'zh-CN': '启用该白名单', en: 'Enable this whitelist entry' },
  ...
  importTitle: { 'zh-CN': '批量导入白名单', en: 'Batch Import Whitelist' },
  importStepHint: {
  ...
  'type.TEMPORARY': { 'zh-CN': '临时车', en: 'Temporary' },
  'type.RESERVED': { 'zh-CN': '预约车', en: 'Reserved' },
  'type.VIP': { 'zh-CN': 'VIP 车辆', en: 'VIP Vehicle' },
  'type.OWNER': { 'zh-CN': '业主车辆', en: 'Owner Vehicle' },
  'type.MONTHLY': { 'zh-CN': '月租车辆', en: 'Monthly Vehicle' },
  'type.OTHER': { 'zh-CN': '其他', en: 'Other' }
}
...
/** 点击车牌复制车牌号到剪贴板。 */
async function handleCopyPlate(row: WhitelistItem) {
...
async function loadList() {
  ...
  loading.value = true
  try {
    const data = await request.get<never, PageResult<WhitelistItem>>(
      `/lots/${selectedLotId.value}/whitelist-vehicles`,
      {
        params: {
...
const form = reactive({
  ...
  ownerName: '',
  type: 'OTHER' as VehicleTypeValue,
  phone: '',
  department: '',
  remark: '',
  startTime: '',
...
async function handleSave() {
  ...
  try {
    if (isEditing.value && editingId.value != null) {
      await request.put(`/lots/${selectedLotId.value}/whitelist-vehicles/${editingId.value}`, payload)
    } else {
      await request.post(`/lots/${selectedLotId.value}/whitelist-vehicles`, payload)
    }
...
async function handleDelete(row: WhitelistItem) {
  try {
  ...
  try {
    await request.delete(`/lots/${selectedLotId.value}/whitelist-vehicles/${row.id}`)
...
async function handleDownloadTemplate() {
  ...
  try {
    await downloadFile(
      `/lots/${selectedLotId.value}/whitelist-vehicles/import-template`,
...
async function handleExport() {
  ...
  try {
    ...
    await downloadFile(
      `/lots/${selectedLotId.value}/whitelist-vehicles/export?plate=${encodeURIComponent(plate)}`,
      'whitelist-vehicles.xlsx'
    )
...
async function handleImport() {
  ...
  try {
    ...
    const data = await request.post<never, { imported: number; skipped: number }>(
      `/lots/${selectedLotId.value}/whitelist-vehicles/import`,
      fd
    )
  ...
  <section class="wl-page">
    ...
    <el-card v-else class="panel" shadow="never">
      ...
      <el-table v-loading="loading" :data="rows" stripe>
        ...
        <el-table-column :label="t('thType')" min-width="110">
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
        ...
        <el-form-item :label="t('lblType')" prop="type">
