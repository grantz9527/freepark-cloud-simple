...
const d: BiDict = {
  ...
  exportNeedLot: { 'zh-CN': '请先选择车场再导出', en: 'Select a parking lot before exporting' },
  exportSuccess: { 'zh-CN': '已开始下载', en: 'Download started' },
  exportFailed: { 'zh-CN': '导出失败，请重试', en: 'Export failed, try again' },
  tip: {
    'zh-CN': '按站点时区自然日统计各车场成功收款与退款。仅计入支付记录中状态为成功的请求；实收 = 收款 − 退款。合计不受分页影响。',
    en: 'Daily lot totals in the site timezone, counting successful payment and refund records only. Net = paid − refunded. Totals ignore pagination.'
  },
...
async function loadRows() {
  ...
  try {
    ...
    const data = await request.get<never, StatsResult>('/pay-records/lot-daily-stats', {
      params: {
        lotId: filters.lotId,
...
async function handleExport() {
  if (!filters.lotId) {
    ElMessage.warning(t('exportNeedLot'))
    return
  }
  const range = timeRange.value ?? defaultRange()
  ...
  try {
    ...
    const response = await fetch(`/api/pay-records/lot-daily-stats/export?${params.toString()}`, {
      headers: {
    ...
    const lot = lots.value.find((item) => item.id === filters.lotId)
    const fallback = `lot-stats_${lot?.name ?? filters.lotId}_${range[0]}_${range[1]}.csv`
...
<template>
  <section class="page-stack">
    <div class="page-card toolbar">
      ...
      <el-tooltip :content="t('exportNeedLot')" :disabled="!!filters.lotId" placement="top">
        <span>
          <el-button :disabled="!filters.lotId" :loading="exporting" @click="handleExport">
            {{ t('export') }}
          </el-button>
        </span>
      </el-tooltip>
    ...
    <div class="page-card">
      <el-table v-loading="loading" :data="rows" stripe style="width: 100%">
        <el-table-column prop="statDate" :label="t('statDate')" min-width="120" />
        <el-table-column :label="t('lot')" min-width="160" show-overflow-tooltip>
          <template #default="{ row }">
            {{ row.lotName || '—' }}
          </template>
        </el-table-column>
        <el-table-column :label="t('pay')" min-width="120" align="right">
          <template #default="{ row }">
            <span class="amount-pay">{{ moneyText(row.payYuan) }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="t('wechat')" min-width="120" align="right">
          <template #default="{ row }">
            {{ moneyText(row.wechatYuan) }}
          </template>
        </el-table-column>
        <el-table-column :label="t('alipay')" min-width="120" align="right">
          <template #default="{ row }">
            {{ moneyText(row.alipayYuan) }}
          </template>
        </el-table-column>
        <el-table-column :label="t('cash')" min-width="120" align="right">
          <template #default="{ row }">
            {{ moneyText(row.cashYuan) }}
          </template>
        </el-table-column>
        <el-table-column :label="t('refund')" min-width="120" align="right">
          <template #default="{ row }">
            <span class="amount-refund">{{ moneyText(row.refundYuan) }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="t('net')" min-width="120" align="right">
          <template #default="{ row }">
