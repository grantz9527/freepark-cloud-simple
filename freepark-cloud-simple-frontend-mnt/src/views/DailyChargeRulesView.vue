...
const d: BiDict = {
  title: { 'zh-CN': '每日制计费规则', en: 'Daily Billing Rules' },
  hintTitle: {
    'zh-CN': '规则口径',
    en: 'How rules apply'
  },
  hintBody: {
    'zh-CN': '每日制模板通过「周计划」逐天（周一~周日）控制当天是否收费：未配置任何信息的天默认当天全天按费率收费；配置了收费时段的天仅在该时段内计费（每天可配多个时段）；某天标记为「当天免费」则整天不收费。计价与 24 小时制类似：支持「固定周期单价」或选用全局「计费周期」方案，单日累计不超过「每日封顶」金额（0 = 不封顶）。本页维护的是全局计费模板，不直接归属车场或车牌颜色；在「车场管理」每行的“计费配置”中按车场选用模板，并指定适用车牌颜色与生效起止日期（同车场同颜色的生效区间不可重叠）。',
    en: 'Daily templates control charging day by day (Mon–Sun) via a weekly plan: a day with no settings charges the whole day at the template rate; a day with windows charges only inside them (multiple windows per day allowed); a day marked “free” is not charged at all. Pricing mirrors the 24-hour mode: a fixed cycle (length × unit price) or a global Billing Cycle profile, capped by the daily ceiling (0 = no cap). This page keeps global templates that are not bound to any lot or color; lots pick them in “Billing Config” under Lot Management, choosing the plate color and an effective date range (a lot cannot have overlapping ranges for the same color).'
  },
  ...
  capText: { 'zh-CN': '每日封顶', en: 'Daily cap' },
  ...
  formCap: { 'zh-CN': '每日封顶', en: 'Daily cap' },
  formCapTip: {
    'zh-CN': '单日收费时段内累计费用不超过该金额，单位见「收费金额单位」配置。0 = 不封顶。',
    en: 'Total fee never exceeds this amount within a charge day; unit per the fee-currency setting. 0 = no cap.'
  },
...
function planFromSlots(slots: DailySlot[]): PlanDay[] {
...
function capText(row: RuleRow): string {
  return row.capPerDayYuan > 0
    ? `${t('capText')} ${fmtMoney(row.capPerDayYuan)} ${moneyUnit.value}`
    : t('noCap')
}
...
async function loadRules(): Promise<void> {
  loading.value = true
  try {
    const list = await request.get<never, RuleRow[]>('/billing/daily-rules')
    rows.value = list ?? []
  } catch (error) {
...
function openEditDialog(row: RuleRow): void {
  ...
  form.capPerDayYuan = Number(row.capPerDayYuan)
...
async function handleSubmit(): Promise<void> {
  ...
  const cap = Number(form.capPerDayYuan ?? 0)
  if (
  ...
  try {
    if (editing.value) {
      await request.put<never, RuleRow>(`/billing/daily-rules/${editing.value.id}`, payload)
      ElMessage.success(t('updateSuccess'))
    } else {
      await request.post<never, RuleRow>('/billing/daily-rules', payload)
...
async function handleDelete(row: RuleRow): Promise<void> {
  ...
  try {
    await request.delete<never, void>(`/billing/daily-rules/${row.id}`)
  ...
  <section class="rules-page">
    <el-card class="panel" shadow="never">
      ...
      >
        ...
        <el-table-column :label="t('colFee')" min-width="190">
          <template #default="{ row }">
            <div class="fee-cell">
        ...
        <el-table-column :label="t('colFree')" width="100" align="center">
          <template #default="{ row }">{{ minuteText((row as RuleRow).freeMinutes) }}</template>
        </el-table-column>

        <el-table-column :label="t('colGrace')" width="110" align="center">
          <template #default="{ row }">{{ minuteText((row as RuleRow).graceMinutes) }}</template>
        </el-table-column>

        <el-table-column :label="t('colPlan')" min-width="250">
          <template #default="{ row }">
