...
const d: BiDict = {
  ...
  sectionKeyHint: {
    'zh-CN': '请求签名使用应用私钥；响应与异步通知验签使用支付宝公钥。密钥工具导出的多为无头尾纯文本 .txt，可直接上传；也支持 PEM。保存时后端自动校验格式，文件内容仅保存不回显。',
    en: 'Requests are signed with the app private key; Alipay responses and async notifications are verified with the Alipay public key. The key tool usually exports bare .txt without PEM headers—upload as-is; PEM is also accepted. The backend validates on save. Files are stored but never returned.'
  },
  ...
  privateKeyFileHint: {
    'zh-CN': '支付宝密钥工具导出的应用私钥 .txt（PKCS8 裸 Base64，无需 BEGIN/END 头尾；也支持 PEM）',
    en: 'App private key .txt from the Alipay key tool (PKCS8 bare Base64, no BEGIN/END headers required; PEM also OK)'
  },
  privateKeySaveHint: {
