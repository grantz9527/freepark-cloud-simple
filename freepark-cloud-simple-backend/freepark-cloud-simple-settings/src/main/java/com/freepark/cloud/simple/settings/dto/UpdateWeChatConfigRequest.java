 ...
 * （无法清空）。商户 API 证书走 {@link UpdateWeChatConfigUploadRequest} 文件上传接口。
 * 是否开放微信支付由「系统配置 → 收费方式」统一控制，本请求不含启停开关。</p>
 *
 * @param mchId           微信支付商户号（纯数字，可空）
 * @param mchName         商户名称（可空）
 * @param mchCertSerialNo 商户序列号（证书序列号，可空；上传证书时会被解析值覆盖）
 * @param mchApiKey       商户 API 密钥（可选更新，仅字母数字）
 * @param mpAppId         缴费授权公众号 AppID（形如 wx + 16 位，可空）
